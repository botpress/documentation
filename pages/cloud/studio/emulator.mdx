...
The Emulator shows the status of the conversation and the different steps that are being or have been executed under the hood to get the bot's response. They are a simpler version of the [Event Debugger](./event-debugger-logs-json)'s summary. Here's some possible logs:
...
- No content matched the query: Accompanies the previous status. The bot didn't try to find an answer in the Knowledge Base.
