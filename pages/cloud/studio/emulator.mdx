...
<Callout type="info">
  **Note** <br />
  To visualize the actual user experience, use the Share button. All the messages you send with the Emulator come from the
  same user from the bot's perspective. Conversations won't be saved and the [bot invocations](../admin-dashboard/usage/#invocation-calls)
  won't be counted, but AI operations will.
</Callout>
...
The Emulator shows the status of the conversation and the different steps that are being or have been executed under the hood to get the bot's response. It's a simpler version of the [Event Debugger](./event-debugger-logs-json). Here's some possible statuses and steps:
...
- No content matched the query: Accompanies the previous status. The bot didn't tru to find an answer in the Knowledge Base.
- Knowledge Base answer: If a text is displayed automatically without the need of a Text card, it's a Knowledge Base answer. It will contain the citations to help you [debug](./knowledge-base/#debugging).
...
- Capture variable failed: The bot wasn't able to extract information and has surpassed the limit of [retries](../toolbox/capture-information/#retries).
