...
1. To make your bot send real data to Zapier, you'll find a "Send to Zapier" action when editing your bot in the Botpress Studio which you can use to send data to your zap:
   ![Zapier action](/docs/content/zapier-action.png)
   It's recommended that you send your data in JSON format so you can send multiple value at once, as Zapier will automatically parse it. For example, you could use the following expression to send a JSON object with multiple values in the "Trigger Data" field of the "Send to Zapier" action:
   ```
