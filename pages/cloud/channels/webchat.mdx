...
| `containerWidth`             | Sends an event to the parent container with the width provided                                                                                                                          | `360`                  | string             | number |
| `layoutWidth`                | Sets the width of the web chat                                                                                                                                                          | `360`                  | string             | number |
...
### Send data from website to web chat
...
You can receive data from the web chat in your website by using `Get User Data` Card in the flow.

#### User ID

Pass the `userId` to the web chat to receive the data from the web chat using `{{event.userId}}` and store it in a variable.


### customUser Object

You can pass the `customUser` object to the web chat to pass the user data to the bot.
...
window.botpressWebChat.init({
  ...
  customUser: {
    ...
    userToken: 'token returned from the USER.CONNECTED post to parent',
  },
})
```
