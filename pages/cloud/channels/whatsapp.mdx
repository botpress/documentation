...
<Callout type="info">
  **Note**
  <br />
  If you are using the Botpress SDK, the `client` in the code example above is an instance of the [Botpress
  Client](https://www.npmjs.com/package/@botpress/client) that is always provided as a prop to the bot handlers (e.g.
  message handler, event, conversation, state expiry, etc.). But if you you can also run this code on a Node.js
  environment where you can install the client package.
</Callout>
