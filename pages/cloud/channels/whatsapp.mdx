...
- Interactive feedback is mapped to Text Object.
- Botpress does not currently process the rest of the types

## Tips

- To get the phone number of the user, use the variable:

  `{{ event.tags.conversation['whatsapp:userPhone'] }}`.

  This number also contains the country code and has no spaces, dashes or signs.

- To get the phone number ID of the user, use the variable:

  `{{ event.tags.conversation['whatsapp:phoneNumberId'] }}`

## Receiving a Location

When WhatsApp shares a location it is mapped to the Botpress Location object and the location information is found in the event.payload object. In the conversation, you could send a text message card asking to send the location using the built-in feature, then have a  "Wait for Message" card to await the user response. 

Once you get the response, you can access the properties of the location using the below:
...
For instance your node could have: 
[Text card]: `Where are you? Please use Whatsapp's send location feature.`
[Wait for Message card]
[Text card]: `Thank you! You live at {{event.payload.title}}, {{event.payload.address}} with longitude {{event.payload.longitude}} and latitude {{event.payload.latitude}}`
...
Then you just need to pass the user's phone number, Whatsapp template name, template language (optional), and list of template variable values as JSON (if any, optional) to proactively start a Whatsapp conversation with a user:
...
> **Note**: Template variables need to be passed as a JSON array of string or numeric values representing the list of variable values, as Whatsapp only allows referring to them by their position in your message template (e.g. `{{1}}` for the first variable, `{{2}}` for the second variable, and so on).
...
#### Accessing the Conversation ID

If you need to access the ID of the Botpress conversation created by this action, you can choose to store the output value of this action in a variable. The output value of this action will be an object with the following structure:
...
So for example, if you choose to store the output value of this action in a variable named `whatsappResult`, you can use `{{ workflow.whatsappResult.conversationId }}` in input fields of the Studio (where supported) to insert the conversation ID, or use `workflow.whatsappResult.conversationId` to access it in code.
