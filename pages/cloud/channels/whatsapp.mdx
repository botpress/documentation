...
> **Tip**: If you need to access the ID of the Botpress conversation created by this action, you can choose to store the output value of this action in a variable. The output value of this action will be an object with the following structure:
...
So for example, if you choose to store the output in a variable named `whatsappResult`, you can use `{{ workflow.whatsappResult.conversationId }}` in input fields of the Studio (where supported) or use `workflow.whatsappResult.conversationId` to access the conversation ID in code.
