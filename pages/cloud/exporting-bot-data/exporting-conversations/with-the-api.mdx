...
<Callout type="info">
  ...
  next batch of conversations. The `nextToken` property is returned in the `meta` object of the response. There are two
  ways to use this property: with a loader function or with a finite loop.
</Callout>

## 2. Getting all Conversations from the API

Now that you know the methods of getting the first conversations from the API, let's see how to get all of them. There are two ways to do this: using a custom loader function or using a finite loop. The first method is recommended for bots with a large number of conversations or if you are displaying the conversations in an user interface, while the second is recommended for bots with a small number of conversations or if you need to get all conversations at once in a backend.

### Using a custom loader function
...
### Using a finite Loop (advanced)

Use a loop to fetch all conversations at once. This is not recommended for bots with a large number of conversations as it may take a long time to complete and may cause timeouts. The use of a library like
