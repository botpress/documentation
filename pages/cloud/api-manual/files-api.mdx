...
const res = await client.searchFiles({
  query: userQuestion,
  contextDepth: 1, // you can increase this number to pass more context to the model for each matching passage
})

const relevantInformation = res.data.passages.map(passage => passage.content).join("\n\n----------\n\n")

const answer = await openai.chat.completions.create({
    model: 'gpt-3.5-turbo', // you can change this for any other OpenAI model available here: https://platform.openai.com/docs/models
    messages: [
        {
            role: 'system',
