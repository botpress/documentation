...
- **Use**: Retrieve details about a specific support ticket.
- **Input**: `Ticket ID` from the ticket URL (e.g., if the URL is `https://companyname.zendesk.com/agent/tickets/1`, the `Ticket ID` is `1`).
...
- **Use**: Close a resolved ticket in Zendesk.
- **Input**: `Ticket ID` from the ticket URL.
...
- **Use**: Fetch recent comments on a ticket.
- **Input**: `Ticket ID` from the ticket URL.
...
- **Use**: Search for a customer in Zendesk.
- **Input**: `Search Query` which can be a name, email, or phone number.
...
- **Use**: Provide a list of available support agents.
- **Option**: `Is Online` checkbox to filter for agents currently online.
...
- **Use**: Change the requester of a ticket.
- **Inputs**:
  - `Conversation ID` from the `Get Ticket Conversation` card.
  - `Requester ID` from the requester's profile URL in Zendesk (e.g., `https://companyname.zendesk.com/agent/users/1` for `Requester ID` `1`).
