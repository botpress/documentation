...
1. Adding an Expression card with the label "Always" and the generated code `true`. When the conversation reach this card, it will always flow to the node that you connected the Expression to, since the expression value is always true.
<br />
2. Creating a node and adding a transition line from the default port (that's next to the "Add Card" button) to the desired node. When the conversation reaches the end of the first node, it will always flow to the one that you connected the default port to.
