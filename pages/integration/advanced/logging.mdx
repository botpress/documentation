...
In Botpress Integrations, logging is an essential functionality used for error tracing, tracking down issues, debugging, and keeping track of the bot's actions. The `integrationLogger` object provided by Botpress offers this logging functionality, allowing you to log different severity levels of messages.

The `integrationLogger` is designed for developers or technical personnel dealing with bot integrations in Botpress. It has four methods, namely `info`, `warn`, `error`, and `debug`, each catering to different levels of logging severity.
...
```javascript
integrationLogger.forBot().info(message, ...optionalParams)
```
...
```javascript
integrationLogger.forBot().warn(message, ...optionalParams)
```
...
```javascript
integrationLogger.forBot().error(message, ...optionalParams)
```
...
```javascript
integrationLogger.forBot().debug(message, ...optionalParams)
```
...
```javascript
integrationLogger.forBot().info('This is an informational message', {data: 'Some relevant data'})
```
