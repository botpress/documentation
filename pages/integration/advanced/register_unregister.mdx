...
`register` is fired when the integration is enabled. It's essential to note that `register` can be triggered multiple times, so it's crucial to ensure that your integration handles this appropriately. 
