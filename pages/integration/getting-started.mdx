...
<Callout type="warning">
  **Integration Names** <br/>
  The integration names are unique. If you try to create an integration with the
  same name as an existing one, the CLI will throw an error.
</Callout>
<br/>
...
Firt, start ngrok on any available port:

```bash
ngrok http 3001
```

This should output a URL that looks like this: `https://<random-string>.ngrok.io`. Copy it.

Then, run the integration in dev mode:

```bash
bp dev --ngrok-url <ngrok-url> --port 3001
```
...
Then, run the bot in dev mode:

```bash
bp dev --ngrok-url <ngrok-url> --port 3002
```

## Iterate

At this point in time, you should have 4 different terminals running: one for the integration, one for the bot and two for ngrok.
