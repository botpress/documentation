...
This integration allows Botpress to communicate with ClickUp and perform various actions like creating tasks and comments and listening to ClickUp events through webhooks. Follow the steps below to set up and utilize the integration.
Prerequisites

- Ensure Node is installed. Check with `node --version`. Please use Node version `18`.
- Ensure pnpm is installed and is on version 8. Check with `pnpm --version`. If not, install with `brew install pnpm`.

### Setup

1. `pnpm dlx @botpress/cli init`  - For Initialising the integrations template
2. `pnpm init`  - to initialise empty project. Copy **tsconfig.json** from template to the root of this project

### Installing Necessary Packages
    - `pnpm install @botpress/sdk @botpress/client zod axios` - sdk for botpress, client for clickup, zod for validation, axios for http requests
    - `pnpm install -D @botpress/cli typescript @types/node ts-node` - dev dependencies for botpress cli, typescript, node, ts-node
```json
"scripts": {
    "version": "bp --version",
    "login": "bp login",
