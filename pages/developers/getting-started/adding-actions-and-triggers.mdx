...
When you install an integration in your bot in Botpress Studio, you can add custom cards that run any NodeJS code and triggers that listen for external events.

For this tutorial, we will add the capability for a bot to create a trackable URL that notifies the creator when it is used.
