...
The `WebchatProvider` component also accepts a `theme` prop that is an object that will style the webchat. Read the [extending Botpress themes](theming/extending-botpress-themes) to generate the `theme object` and `stylesheet`.
...
<Demo src="https://stackblitz.com/edit/bp-webchat-template-c51e4w?embed=1&file=src%2FApp.tsx&hideNavigation=1" title="React Library Demo" />
